...
- “放”与“取”
  - 放：根据空间大小，把补码截断，放入内存（决定了开辟的内存大小）
    - `float`: 4字节
    - `double`: 8字节
  - 取：根据类型判断，是否要转化为原码（决定了如何解析数据）
    - 示例：
...
宽度和精度(“%*.*s”, width, precision, str)————用“*”占位宽度和精度，用“.”分割宽度和精度

- ​    精度（小数点或者字符串的输出字符个数）：%.xf————保留x小数 ；%.4s————hellowrold只输出hell

  ​    1.245，%2.2d————“⬛️1.24” ；

- ​    宽度（填不满时用空格补充，默认右对齐）：%2d————“⬛️1” “⬛️2” “11” ；%-2d————“1⬛️” ；
