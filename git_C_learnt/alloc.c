...
// 内存分配方法选择：
// #define _ALLOC_BEST_FIT_	// 最适应法
#define _ALLOC_FIRST_FIT_	// 首次适应法
...
__uint32_t AllocCnt = 0;	// 用于测试的全局变量，监控程序内存使用情况

/*-------------------------------------------------------------------*/
void wjq_free_t(void *ap)
{
	/* bp: 要释放的内存块指针 */
	/* p: 遍历空闲链表的指针，freep的前一个空闲块指针 */
	ALLOC_HDR *bp, *p;
	...
	// 找到需要释放的内存的前后空闲块（比较内存块位置的地址大小），不满足时进循环
	...
	/*判断是否能跟一个块合并（两个内存块紧邻），能合并就合并，不能合并就用链表连起来*/
	// 没有用if else结构，当执行完两个if之后，bp一定会被加入链表——>分析的时候看的不够全面？
...
void *wjq_malloc_t(unsigned nbytes)
{
	...
	if ((prevp = freep) == NULL)
	{
		p = (ALLOC_HDR *)
			__HEAP_START;
		p->s.size = (((__uint32_t)__HEAP_END - (__uint32_t)__HEAP_START) / sizeof(ALLOC_HDR));
		p->s.ptr = &base;
		base.s.ptr = p;
		base.s.size = 0;
		prevp = freep = &base;
